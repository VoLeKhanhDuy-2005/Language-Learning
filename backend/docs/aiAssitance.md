...
2. Kiểm tra tính hợp lệ:
   - Nếu không có `question`, API trả về lỗi `400 Bad Request` với thông báo: _"Bắt buộc nhập câu hỏi"_.
...
1. **Trích xuất từ khóa (Keyword Extraction):**
   ...
   - Nếu `keywords` rỗng: Trả về lỗi tự định nghĩa: _"Không tìm thấy từ khóa hợp lệ trong câu hỏi để tra cứu hệ thống."_ (`isValidQuestion: false`).
...
2. **Truy vấn Dữ liệu Hệ thống (Query MinLish Data):**
   ...
   - Nếu tìm thấy từ vựng chính, hệ thống sẽ tiếp tục lấy thêm dữ liệu liên quan để làm giàu ngữ cảnh:
     - _10 thẻ từ vựng liên quan cùng Topic_.
     - _10 thẻ từ vựng liên quan cùng Deck_.
     - _10 bài học (Lesson)_ có chứa từ khóa này trong tiêu đề hoặc mô tả.
...
3. **Sinh Câu Trả Lời (Generate Response):**
   - Kiểm tra `contextData`:
     - Nếu **không tìm thấy** dữ liệu nào khớp trong Database: Trả về thông báo: _"Hệ thống MinLish hiện tại chưa có dữ liệu nào khớp với câu hỏi của bạn. Hãy thử chuyển sang chế độ tìm kiếm trên mạng!"_ (`isValidQuestion: true`).
