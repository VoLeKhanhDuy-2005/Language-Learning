...
import {
  exportAdminTopicCardsApi,
  importAdminTopicCardsApi,
} from "../../adminApi";
...
function AdminImportExportModal({ deckId, topic, onClose, onImportSuccess }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `vocab_${(topic.name || "topic").replace(/\s+/g, "_")}_export.xlsx`,
      );
  ...
  const handleImportSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(
        err.response?.data?.message || err.message || t("admin.importFailed"),
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="admin-ie-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="admin-ie-header">
          <div className="admin-ie-header-left">
            ...
            <div>
              <h3 className="admin-ie-title">
                {t("admin.importExportModalTitle")}
              </h3>
              <p className="admin-ie-subtitle">
                {t("admin.importExportSubtitle")} <strong>{topic?.name}</strong>
              </p>
            </div>
          </div>
          <button
            className="admin-ie-close-btn"
            onClick={onClose}
            aria-label={t("admin.cancelBtn2")}
          >
            <svg
              viewBox="0 0 24 24"
              width="20"
          ...
          {activeTab === "export" ? (
            ...
            <div className="admin-ie-export-content">
              <div className="admin-ie-info-card">
                ...
                <div>
                  <h4 className="admin-ie-info-title">
                    {t("admin.exportInfoTitle")}
                  </h4>
                  <p className="admin-ie-info-desc">
                    {t("admin.exportInfoDesc").replace(
                      "{{topic}}",
                      topic?.name || "",
                    )}
                    <code>term</code>, <code>translation</code>,{" "}
                    <code>pos</code>, <code>phonetics</code>,{" "}
                    <code>explanation_vi</code>, <code>explanation_en</code>,{" "}
                    <code>examples_vi</code>, <code>examples_en</code>,{" "}
                    <code>imageUrl</code>.
                  </p>
                </div>
              </div>
              ...
              <div className="admin-ie-columns-preview">
                <p className="admin-ie-columns-label">
                  {t("admin.exportColumnsLabel")}
                </p>
                <div className="admin-ie-columns-list">
                  {[
                    ...
                    <span
                      key={col}
                      className={`admin-ie-col-tag ${col.endsWith("*") ? "required" : ""}`}
                    >
                      {col.replace("*", "")}
                      {col.endsWith("*") && (
                        <span className="admin-ie-col-required">*</span>
                      )}
                    </span>
                  ))}
                </div>
            ...
            <div className="admin-ie-import-content">
              ...
              {!selectedFile ? (
                ...
                >
                  ...
                  <p className="admin-ie-dropzone-text">
                    {t("admin.dropzoneText")}
                  </p>
                  <p className="admin-ie-dropzone-subtext">
                    {t("admin.dropzoneSubtext")}{" "}
                    <span className="admin-ie-dropzone-highlight">
                      {t("admin.dropzoneHighlight")}
                    </span>
                  </p>
                </div>
              ) : (
                <div className="admin-ie-file-selected">
                  <div className="admin-ie-file-info">
                    ...
                    <div>
                      <div className="admin-ie-file-name">
                        {selectedFile.name}
                      </div>
                      <div className="admin-ie-file-size">
              ...
              <div className="admin-ie-mode-group">
                <label className="admin-ie-mode-label">
                  {t("admin.importModeLabel")}
                </label>
                  ...
                  {["append", "replace", "upsert"].map((mode) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="admin-ie-mode-desc">
              ...
              {importResult && (
                <div className="admin-ie-result">
                  <h4
                    className="admin-ie-result-title"
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: "8px",
                  ...
                  <div className="admin-ie-result-grid">
                    <div className="admin-ie-result-item">
                      <span className="admin-ie-result-label">
                        {t("admin.importResultTotalRows")}
                      </span>
                      <span className="admin-ie-result-val">
                        {importResult.totalRows}
                      </span>
                    </div>
                    <div className="admin-ie-result-item">
                      <span className="admin-ie-result-label">
                        {t("admin.importResultInserted")}
                      </span>
                      <span className="admin-ie-result-val success">
                        {importResult.inserted}
                      </span>
                    </div>
                    <div className="admin-ie-result-item">
                      <span className="admin-ie-result-label">
                        {t("admin.importResultUpdated")}
                      </span>
                      <span className="admin-ie-result-val">
                    ...
                    <div className="admin-ie-result-item">
                      <span className="admin-ie-result-label">
                        {t("admin.importResultFailed")}
                      </span>
                      <span className="admin-ie-result-val error">
                        {importResult.failed}
                      </span>
                    </div>
                  </div>
                  ...
                  {importResult.errors && importResult.errors.length > 0 && (
                    <div className="admin-ie-errors">
                      <p className="admin-ie-errors-title">
                        {t("admin.importResultErrorsTitle")}
                      </p>
                      <ul className="admin-ie-errors-list">
                        {importResult.errors.map((err, idx) => (
                          <li key={idx} className="admin-ie-error-item">
                            <span className="admin-ie-error-row">
                              {t("admin.importResultErrorRow")} {err.row}:
                            </span>
                            {err.reason}
                          </li>
                        ))}
