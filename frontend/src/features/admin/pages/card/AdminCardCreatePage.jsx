...
function AdminCardCreatePage({ deckId, topicId, onNavigate }) {
  ...
  const [editingPronunciationIndex, setEditingPronunciationIndex] =
    useState(null);
  const [pronunciationDeleteIndex, setPronunciationDeleteIndex] =
    useState(null);
  ...
  const savePronunciation = () => {
    ...
    setForm((prev) => ({
      ...prev,
      phonetics:
        editingPronunciationIndex === null
          ? [...prev.phonetics, { ...phoneticDraft }]
          : prev.phonetics.map((item, index) =>
              index === editingPronunciationIndex
                ? { ...item, ...phoneticDraft }
                : item,
            ),
    }));
  ...
  const confirmRemovePronunciation = () => {
    ...
    setForm((prev) => ({
      ...prev,
      phonetics: prev.phonetics.filter(
        (_, itemIndex) => itemIndex !== pronunciationDeleteIndex,
      ),
    }));
  ...
  const handleSubmit = async () => {
    ...
    if (!form.translation.trim())
      nextErrors.translation = t("admin.cardTranslationRequired");
      ...
      <div className="admin-card-form-top">
        <div>
          <div className="admin-breadcrumbs">
            ...
            >
              {deck?.title || t("admin.deckNotFound")}
            </span>
            <span className="breadcrumb-separator">›</span>
            <span
              className="breadcrumb-link"
            ...
            <span className="breadcrumb-active">
              {t("admin.cardCreateTitle")}
            </span>
          </div>
          <h1 className="admin-card-form-title">
            {t("admin.cardCreateTitle")}
          </h1>
        </div>
      ...
      <div className="admin-card-form-grid">
        ...
        <section className="admin-card-form-panel admin-card-detail-panel">
          ...
          <div className="admin-card-two-columns">
            <label className="admin-card-field">
              <span>{t("admin.cardExplanationEnLabel")}</span>
              <textarea
                value={form.explanationEn}
                onChange={(event) =>
                  updateField("explanationEn", event.target.value)
                }
                rows={4}
            ...
            <label className="admin-card-field">
              ...
              <textarea
                ...
                onChange={(event) =>
                  updateField("explanationVi", event.target.value)
                }
                rows={4}
                ...
                {form.phonetics.map((phonetic, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => openEditPronunciationModal(index)}
                    ...
                    <button
                      ...
                      onClick={() => setPronunciationDeleteIndex(index)}
          ...
          <div className="admin-card-examples">
            <span className="admin-card-section-label-text">
              {t("admin.cardExampleTitle")}
            </span>
            <label className="admin-card-field">
              <span>{t("admin.cardExampleEnLabel")}</span>
              <textarea
                rows={3}
                value={form.exampleEn}
                onChange={(event) =>
                  updateField("exampleEn", event.target.value)
                }
                placeholder='"She is a resilient girl - she won’t be unhappy for long."'
              />
            </label>
            <label className="admin-card-field">
              ...
              <textarea
                ...
                onChange={(event) =>
                  updateField("exampleVi", event.target.value)
                }
                placeholder='"Cô ấy là một cô gái kiên cường..."'
              />
            </label>
          </div>
        </section>
