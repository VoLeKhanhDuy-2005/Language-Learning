        ...
        <div className="admin-dash-stat-grid">
          {statCards.map((card) => (
            <div key={card.key} className="admin-dash-stat-card">
              <div className="admin-dash-stat-icon-wrap">{card.icon}</div>
              <div className="admin-dash-stat-body">
                ...
                <div className="admin-dash-stat-bottom">
                  <span className="admin-dash-stat-value">
        ...
        <div className="admin-dash-analytics-card">
          <div className="admin-dash-analytics-header">
            <div>
              <h2 className="admin-dash-section-title">
                {t("admin.dashRegistrationChart")}
              </h2>
              <p className="admin-dash-section-desc">
                {t("admin.dashAnalyticsDesc", {
                  total: formatCount(totalRegistrations),
                })}
              </p>
            </div>
            <select
          ...
          <div className="admin-dash-chart-container">
            {chartData.length > 0 ? (
              ...
              <div className="admin-dash-chart-empty">
                {t("admin.dashNoChartData")}
              </div>
            )}
          </div>
        </div>
              ...
              {metrics.popularLessons && metrics.popularLessons.length > 0 ? (
                metrics.popularLessons.map((item, idx) => (
                  <button
                    ...
                    onClick={() =>
                      onNavigate &&
                      onNavigate(`/admin/lessons/${item._id}/edit`)
                    }
                  >
                    ...
                    <div className="admin-dash-popular-info">
                      <span className="admin-dash-popular-name">
                        {item.title}
                      </span>
                    ...
                    <svg
                      className="admin-dash-popular-arrow"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2.5"
                ...
                <div className="admin-dash-popular-empty">
                  {t("admin.dashNoPopularData")}
                </div>
              )}
            </div>
          </div>
              ...
              {metrics.popularDecks && metrics.popularDecks.length > 0 ? (
                metrics.popularDecks.map((item, idx) => (
                  <button
                    ...
                    className="admin-dash-popular-item"
                    onClick={() =>
                      onNavigate && onNavigate(`/admin/decks/${item._id}`)
                    }
                  >
                  ...
                  <tbody>
                    {recentActivity.map((item) => (
                      <tr key={item._id}>
                        <td>
                          <span
                            className={`admin-dash-type-badge ${item.type}`}
                          >
                            {item.type === "lesson"
                              ? t("admin.dashTypeLesson")
                              : t("admin.dashTypeDeck")}
                          </span>
                        </td>
                        <td className="admin-dash-table-title-cell">
                          {item.title}
                        </td>
                        <td className="admin-dash-table-muted">
                          {item.author}
                        </td>
                        <td className="admin-dash-table-muted">
