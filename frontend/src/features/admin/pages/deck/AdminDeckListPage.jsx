...
import AdminCardSearchTab from "./AdminCardSearchTab";
...
function AdminDeckListPage({ onNavigate }) {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return params.get("tab") === "cards" ? "cards" : "decks";
  });
  ...
  const handleArchiveToggle = async (deck) => {
    const newStatus =
      deck.status === "archived"
        ...
        : "archived";
    try {
      await updateAdminDeckApi(deck._id, {
        ...deck,
        status: newStatus,
        title: deck.title,
      });
  ...
  return (
    <div className="admin-deck-list-page">
      ...
      >
        <button
          className={`admin-tab-btn ${activeTab === "decks" ? "active" : ""}`}
        ...
        <button
          className={`admin-tab-btn ${activeTab === "cards" ? "active" : ""}`}
      ...
      {activeTab === "decks" ? (
        <>
          ...
          <div className="admin-filter-bar">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((prev) => ({ ...prev, cefrLevelId: e.target.value }))
              }
            >
            ...
            <div className="admin-deck-grid">
              {decks.map((deck) => {
                ...
                return (
                  <div
                    key={deck._id}
                    className={`admin-deck-card ${isArchived ? "archived" : ""}`}
                  >
                    {/* Cover */}
                    ...
                    <div className="admin-deck-content">
                      <div className="admin-deck-meta">
                        {cefrLabels.map((lbl, idx) => (
                          <span
                            key={idx}
                            className="admin-meta-badge cefr-badge"
                          >
                            {lbl}
                          </span>
                        ))}
                        {tagLabels.map((lbl, idx) => (
                          <span
                            key={idx}
                            className="admin-meta-badge tag-badge"
                          >
                            {lbl}
                          </span>
                        ))}
                    ...
                    <div className="admin-deck-actions">
                      <button
                        ...
                        onClick={() =>
                          onNavigate &&
                          onNavigate(`/admin/decks/${deck._id}/edit`)
                        }
                      >
                        <svg
                          viewBox="0 0 24 24"
                      ...
                      <a
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          onNavigate && onNavigate(`/admin/decks/${deck._id}`);
                        }}
              ...
              >
                ...
                <span className="admin-deck-create-label">
                  {t("admin.createDeckCard")}
                </span>
                <span className="admin-deck-create-desc">
                  {t("admin.createDeckCardDesc")}
                </span>
              </button>
            </div>
          )}
            ...
            <div className="admin-pagination">
              ...
              <div className="admin-pagination-controls">
                <Pagination
                  ...
                  onPageChange={(page) => fetchDecks(page)}
                />
              </div>
            </div>
          )}
        ...
        <AdminCardSearchTab onNavigate={onNavigate} />
      )}
    </div>
  );
}
