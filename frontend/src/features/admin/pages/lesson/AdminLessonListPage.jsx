...
function AdminLessonListPage({ onNavigate }) {
  ...
  const fetchLessons = useCallback(
    async (page = 1) => {
      ...
      setError("");
      try {
        const res = await listAdminLessonsApi({
          ...filters,
          page,
          limit: LIMIT,
        });
  ...
  const cefrMap = useMemo(() => {
    return new Map(
      cefrLevels.map((level) => [level._id, level.label || level.code]),
    );
  }, [cefrLevels]);
  ...
  const handleArchiveToggle = async (lesson) => {
    ...
    try {
      await updateAdminLessonApi(lesson._id, {
        ...lesson,
        status: newStatus,
        title: lesson.title,
      });
  ...
  const from =
    pagination.totalItems === 0 ? 0 : (pagination.page - 1) * LIMIT + 1;
            ...
            <table className="admin-lesson-table">
              <thead>
                <tr>
                  ...
                  <th className="admin-lesson-actions-head">
              ...
              <tbody>
                {lessons.map((lesson) => {
                  ...
                  return (
                    <tr
                      key={lesson._id}
                      className={lesson.status === "archived" ? "archived" : ""}
                    >
                      <td>
                        <div className="admin-lesson-info">
                          ...
                          <div className="admin-lesson-copy">
                            ...
                            <span>
                              {lesson.description || t("admin.noDescription")}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td>
                        <div className="admin-lesson-badge-list">
                          {cefrLabels.length > 0 ? (
                            cefrLabels.map((label) => (
                              <span
                                key={label}
                                className="admin-lesson-badge cefr"
                              >
                                {label}
                              </span>
                            ))
                        ...
                        <div className="admin-lesson-badge-list">
                          {tagLabels.length > 0 ? (
                            tagLabels.map((label) => (
                              <span
                                key={label}
                                className="admin-lesson-badge tag"
                              >
                                {label}
                              </span>
                            ))
                        ...
                        <div className="admin-lesson-badge-list">
                          {modes.length > 0 ? (
                            modes.map((mode) => (
                              <span
                                key={mode}
                                className="admin-lesson-badge mode"
                              >
                                {getModeLabel(mode)}
                              </span>
                            ))
                      ...
                      <td>
                        <span
                          className={`admin-lesson-status ${lesson.status || "draft"}`}
                        >
                      ...
                      <td>
                        <span className="admin-lesson-date">
                          {formatDate(
                            lesson.updatedAt ||
                              lesson.publishedAt ||
                              lesson.createdAt,
                      ...
                      <td>
                        <div className="admin-lesson-actions">
                          <button
                            ...
                            onClick={() =>
                              onNavigate &&
                              onNavigate(`/admin/lessons/${lesson._id}/edit`)
                            }
                          >
                            <svg
                              viewBox="0 0 24 24"
