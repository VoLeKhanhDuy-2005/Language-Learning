...
const clearTimeDigit = (value, position, direction) => {
  const targetPosition = moveToEditableTimePosition(
    position + direction,
    direction,
  );
  const chars = clampTimeValue(value).split("");
...
const handleTimeKeyDown = (event, value, setValue, clearError) => {
  ...
  if (event.key === "Backspace" || event.key === "Delete") {
    event.preventDefault();
    const next = clearTimeDigit(
      value,
      selectionStart,
      event.key === "Backspace" ? -1 : 0,
    );
    setValue(next.value);
    clearError();
    setTimeCaret(input, next.position);
...
const isValidTimeValue = (value) =>
  /^\d{2}:\d{2}$/.test(value) && value === clampTimeValue(value);
...
function AdminLessonSegmentsPage({ onNavigate, lessonId }) {
  ...
  useEffect(() => {
    ...
    const selectSegmentByPlayback = () => {
      ...
      if (!activeSegment || activeSegment._id === selectedSegmentIdRef.current)
        return;
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      ...
      if (selectedSegment._id === "new") {
        ...
        // Update existing
        await updateAdminLessonSegmentApi(
          lessonId,
          selectedSegment._id,
          payload,
        );
      ...
      if (createdSegmentId) {
        ...
        const updatedSegment = segmentsData.find(
          (s) => s._id === selectedSegment._id,
        );
        if (updatedSegment) {
      ...
      setErrorMsg(
        code
          ? t("api.error." + code)
          : err.response?.data?.message || t("admin.segmentActionError"),
      );
      setTimeout(() => {
        setErrorMsg("");
      }, 3000);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setErrorMsg(
        code
          ? t("api.error." + code)
          : err.response?.data?.message || t("admin.segmentDeleteError"),
      );
      setTimeout(() => {
        setErrorMsg("");
      }, 3000);
  ...
  const handleSelectAll = () => {
    const allIds = new Set(
      segments.filter((s) => s._id !== "new").map((s) => s._id),
    );
  ...
  return (
    <div className="admin-lesson-segments-page">
      ...
      <div className="admin-segment-breadcrumbs">
        ...
        <span
          className="breadcrumb-link"
          onClick={() => onNavigate(`/admin/lessons/${lessonId}/edit`)}
        >
          {lesson.title}
        </span>
        <svg
        ...
        <span className="breadcrumb-current">
          {t("admin.segmentsBreadcrumb") || "Phân đoạn"}
        </span>
      </div>
      ...
      <div className="admin-segment-grid">
        ...
        <div className="admin-segment-left-col">
          ...
          <div className="admin-segment-col-header">
            ...
            <div className="admin-segment-col-actions">
              ...
              <button
                className="admin-add-segment-btn"
          ...
          {/* Bulk Action Bar */}
          {isSelectMode &&
            segments.filter((s) => s._id !== "new").length > 0 && (
                ...
                <div className="admin-bulk-left">
                  <label className="admin-bulk-checkbox-label">
                    <input
                      ...
                      checked={
                        selectedIds.size ===
                        segments.filter((s) => s._id !== "new").length
                      }
                      onChange={
                        selectedIds.size ===
                        segments.filter((s) => s._id !== "new").length
                ...
                >
                  ...
                  <div className="admin-segment-item-details">
                    <div className="admin-segment-time-row">
                      <span
                        className={`admin-segment-index-badge ${isSelected ? "selected" : ""}`}
                      >
                        #{displayIndex}
                      </span>
                      <span className="segment-time-start">
                        {formatTime(segment.startMs)}
                      </span>
                      <span className="segment-time-separator">
                        {t("admin.segmentTimeSeparator") || "ĐẾN"}
                      </span>
                      <span className="segment-time-end">
                        {formatTime(segment.endMs)}
                      </span>
                    </div>
                    <div className="admin-segment-text-row">
                      <div className="segment-text-box">
                        <span className="segment-text-label">
                          {t("admin.segmentOriginalLabel") ||
                            "BẢN GHI TIẾNG ANH"}
                        </span>
                        <p className="segment-original">
                          {segment.transcript?.original}
                        </p>
                      </div>
                      <div className="segment-text-box">
                        <span className="segment-text-label">
                          {t("admin.segmentTranslationLabel") ||
                            "BẢN DỊCH TIẾNG VIỆT"}
                        </span>
                        <p className="segment-translation">
                          {segment.translation}
                        </p>
                      </div>
                    </div>
                  ...
                  {!isSelectMode && (
                    <div className="admin-segment-item-actions">
                      <button
                        className="admin-segment-edit-btn"
                        title={t("admin.editSegment")}
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSelectSegment(segment);
                        }}
        ...
        <div className="admin-segment-right-col">
          ...
          {selectedSegment ? (
            <form
              className="admin-segment-details-form"
            ...
            >
              <div className="admin-segment-details-header">
                <div>
                  ...
                  {selectedSegment._id !== "new" && (
                    <p className="admin-segment-current-info">
                      {t("admin.segmentNumberLabel") || "Phân đoạn"} #
                      {segments.findIndex(
                        (s) => s._id === selectedSegment._id,
                      ) + 1}{" "}
                ...
                {selectedSegment._id !== "new" && (
                  <button
                    type="button"
                    className="admin-delete-segment-btn"
                    title={t("admin.deleteSegment")}
              ...
              <div className="admin-segment-time-inputs">
                <div className="form-group">
                  <label className="form-label">
                    {t("admin.segmentStartTimeLabel") || "THỜI GIAN BẮT ĐẦU"}
                  </label>
                  <input
                    type="text"
                    placeholder="00:00"
                    ...
                    onPaste={(e) => {
                      handleTimePaste(e, setEditStartTime, () =>
                        setStartTimeError(""),
                      );
                    }}
                    inputMode="numeric"
                  ...
                  />
                  {startTimeError && (
                    <span className="form-error">{startTimeError}</span>
                  )}
                </div>

                <div className="form-group">
                  <label className="form-label">
                    {t("admin.segmentEndTimeLabel") || "THỜI GIAN KẾT THÚC"}
                  </label>
                  <input
                    type="text"
                    placeholder="00:00"
                    ...
                    onKeyDown={(e) => {
                      handleTimeKeyDown(e, editEndTime, setEditEndTime, () =>
                        setEndTimeError(""),
                      );
                    }}
                    onPaste={(e) => {
                      handleTimePaste(e, setEditEndTime, () =>
                        setEndTimeError(""),
                      );
                    }}
                    inputMode="numeric"
                  ...
                  />
                  {endTimeError && (
                    <span className="form-error">{endTimeError}</span>
                  )}
                </div>
              </div>
              ...
              <div className="form-group">
                <label className="form-label">
                  {t("admin.segmentOriginalLabel") || "BẢN GHI TIẾNG ANH"}
                </label>
                <textarea
                  placeholder={
                    t("admin.segmentOriginalPlaceholder") ||
                    "Nhập bản ghi tiếng Anh..."
                  }
                  value={editOriginal}
                  onChange={(e) => {
                ...
                />
                {originalError && (
                  <span className="form-error">{originalError}</span>
                )}
              </div>

              <div className="form-group">
                <label className="form-label">
                  {t("admin.segmentTranslationLabel") || "BẢN DỊCH TIẾNG VIỆT"}
                </label>
                <textarea
                  placeholder={
                    t("admin.segmentTranslationPlaceholder") ||
                ...
                />
                {translationError && (
                  <span className="form-error">{translationError}</span>
                )}
              </div>
              ...
              <div className="admin-segment-form-actions">
                <button
                  type="button"
                  className="admin-cancel-segment-btn"
            ...
            <div className="admin-segment-empty-state">
              ...
              <p>
                {t("admin.selectSegmentPrompt") ||
                  "Chọn một phân đoạn để xem chi tiết hoặc thêm phân đoạn mới."}
              </p>
            </div>
          )}
        </div>
      ...
      <ConfirmModal
        ...
        message={
          t("admin.confirmDeleteSegmentMessage") ||
          "Bạn có chắc chắn muốn xóa phân đoạn này? Hành động này không thể hoàn tác."
        }
      ...
      <ConfirmModal
        ...
        title={
          t("admin.confirmDeleteMultipleSegmentsTitle") ||
          "Xóa các phân đoạn đã chọn"
        }
        message={
          t("admin.confirmDeleteMultipleSegmentsMessage", {
            count: selectedIds.size,
          }) ||
          `Bạn có chắc chắn muốn xóa ${selectedIds.size} phân đoạn đã chọn? Hành động này không thể hoàn tác.`
        }
        confirmText={t("admin.deleteBtn") || "Xóa"}
        cancelText={t("admin.cancelBtn") || "Hủy"}
