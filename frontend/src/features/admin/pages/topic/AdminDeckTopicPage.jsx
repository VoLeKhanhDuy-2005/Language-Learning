...
function AdminDeckTopicPage({ onNavigate, deckId }) {
  ...
  const handleSelectTopic = async (topic) => {
    ...
    if (topic._id && topic._id !== "new") {
      try {
        ...
        const cardsRes = await listDeckCardsApi(deckId, {
          topicId: topic._id,
          limit: 100,
        });
  ...
  const handleAddNewTopic = () => {
    ...
    setErrorMsg("");
    const nextOrder =
      topics.length > 0 ? Math.max(...topics.map((t) => t.order)) + 1 : 1;
    const newTopic = {
      _id: "new",
      name: "",
      order: nextOrder,
    ...
    try {
      ...
      if (selectedTopic._id === "new") {
        ...
        updatedTopicsList = topics.map((t) =>
          t._id === selectedTopic._id ? updatedSelectedTopic : t,
        );
        actionRes = res;
      }
      ...
      const sortedTopics = [...(topicsRes.data || [])].sort(
        (a, b) => a.order - b.order,
      );
      setTopics(sortedTopics);
      ...
      const newSelected =
        sortedTopics.find((t) => t._id === updatedSelectedTopic._id) ||
        updatedSelectedTopic;
      ...
      setErrorMsg(
        err.response?.data?.message || "Có lỗi xảy ra khi lưu chủ đề",
      );
      setTimeout(() => {
        setErrorMsg("");
      }, 3000);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err.response?.data?.message || "Có lỗi xảy ra khi xóa chủ đề",
      );
      setTimeout(() => {
        setErrorMsg("");
      }, 3000);
  ...
  const handleSelectAll = () => {
    const allIds = new Set(
      topics.filter((t) => t._id !== "new").map((t) => t._id),
    );
  ...
  const handleDrop = async (e, index) => {
    ...
    try {
      ...
      if (selectedTopic) {
        const newSelected = sortedTopics.find(
      ...
      setErrorMsg(
        err.response?.data?.message || "Có lỗi xảy ra khi lưu thứ tự",
      );
      setTimeout(() => {
        setErrorMsg("");
      }, 3000);
  ...
  const cefrLabels = (deck.cefrLevelIds || [])
  ...
  return (
    <div className="admin-deck-topic-page">
          ...
          <div className="admin-topic-stat-card">
            ...
            <div className="stat-card-info">
              ...
              <span className="stat-card-value">
                {(deck.cardCount || 0).toLocaleString()}
              </span>
            </div>
          </div>
      ...
      <div className="admin-topic-grid">
        ...
        <div className="admin-topic-left-col">
          <div className="admin-topic-col-header">
            <h2 className="admin-topic-col-title">
              {t("admin.topicStructure")}
            </h2>
              ...
              <div className="admin-bulk-left">
                <label className="admin-bulk-checkbox-label">
                  <input
                    ...
                    checked={
                      selectedIds.size ===
                      topics.filter((t) => t._id !== "new").length
                    }
                    onChange={
                      selectedIds.size ===
                      topics.filter((t) => t._id !== "new").length
          ...
          <div className="admin-topic-list">
            {topics.map((topic, index) => {
              ...
              const displayIndex = String(topic.order ?? index + 1).padStart(
                2,
                "0",
              );
              ...
              return (
                ...
                >
                  ...
                  {/* Index badge */}
                  <div
                    className={`admin-topic-index-badge ${isSelected ? "selected" : ""}`}
                  >
                    {displayIndex}
                  </div>
                  ...
                  <div className="admin-topic-item-details">
                    <h3 className="admin-topic-item-title">
                      {topic.name ||
                        t("admin.newTopicPlaceholder") ||
                        "Chủ đề mới"}
                    </h3>
                    <p className="admin-topic-item-meta">
                      ...
                      {topic.updatedAt && (
                        <>
                          ...
                          {t("admin.updatedAt", {
                            time: formatRelativeTime(topic.updatedAt),
                          })}
                        </>
                      )}
                    </p>
                  ...
                  <div className="admin-topic-item-indicator">
                    {isSelected ? (
                      <span className="admin-topic-active-pill">
                        {t("admin.activeBadge")}
                      </span>
                    ) : (
                      !isSelectMode && (
                        <button
                          className="admin-topic-quick-edit-btn"
          ...
          {!isSelectMode && (
            <div className="admin-topic-reorder-box">
              ...
              <div className="reorder-box-text">
                <h4 className="reorder-title">
                  {t("admin.reorderPlaceholder")}
                </h4>
        ...
        <div className="admin-topic-right-col">
          {selectedTopic ? (
            <form
              className="admin-topic-details-form"
              onSubmit={handleSaveChanges}
            >
              <div className="admin-topic-details-header">
                <h2 className="admin-topic-details-title">
                  {t("admin.topicDetail")}
                </h2>
                {selectedTopic._id !== "new" && (
                  <button
                    type="button"
                    className="admin-delete-topic-btn"
              ...
              <div className="form-group">
                <label className="form-label">
                  {t("admin.topicTitleLabel")}
                </label>
                <input
                  type="text"
                  required
                  placeholder={
                    t("admin.newTopicPlaceholder") || "Nhập tiêu đề chủ đề..."
                  }
                  value={editName}
              ...
              <div className="admin-card-preview-section">
                ...
                <button
                  ...
                  onClick={() => {
                    if (selectedTopic._id !== "new") {
                      onNavigate(
                        `/admin/decks/${deckId}/topics/${selectedTopic._id}/cards`,
                      );
                    }
                  }}
                >
                ...
                {cardsLoading ? (
                  ...
                  selectedTopic._id !== "new" && (
                    <p className="admin-no-cards-text">
              ...
              <button
                ...
                className="admin-save-topic-btn"
              >
            ...
            <div className="admin-topic-empty-state">
              ...
              <p>
                {t("admin.selectTopicPrompt") ||
                  "Chọn một chủ đề để xem chi tiết hoặc sắp xếp."}
              </p>
            </div>
          )}
        </div>
      ...
      <ConfirmModal
        ...
        title={
          t("admin.confirmDeleteMultipleTopicsTitle") ||
          "Xóa các chủ đề đã chọn"
        }
        message={
          t("admin.confirmDeleteMultipleTopicsMessage", {
