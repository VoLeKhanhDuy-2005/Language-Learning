...
function AdminUsersPage({ onNavigate }) {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (res.data) {
          ...
          setPagination(
            res.data.pagination || { page: 1, totalPages: 1, totalItems: 0 },
          );
        } else {
  ...
  const handleConfirmUnlock = async () => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.message || t("api.error.USER_NOT_FOUND");
      setError(errorMsg);
  ...
  const handleBanSubmit = async () => {
    ...
    try {
      const res = await changeAdminUserStatusApi(
        selectedUser._id,
        "banned",
        banReason,
      );
  ...
  const handlePasswordSubmit = async () => {
    ...
    setPasswordError("");
    try {
      const res = await changeAdminUserPasswordApi(
        selectedUser._id,
        newPassword.trim(),
      );
      if (res.data && res.data.code) {
    ...
    <div className="admin-users-page">
      ...
      <div className="admin-users-stats">
        <div className="admin-users-stat-card">
          <div className="admin-users-stat-label">
            {t("admin.dashTotalUsers")}
          </div>
        ...
        <div className="admin-users-stat-card">
          <div className="admin-users-stat-label">
            {t("admin.userStatusBanned")}
          </div>
          <div className="admin-users-stat-value">
            {loadingStats ? "..." : stats.bannedUsers.toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="admin-filter-bar">
        <div className="admin-search-wrap">
          ...
          <input
            ...
            placeholder={
              t("admin.userSearchPlaceholder") || "Tìm kiếm người dùng..."
            }
            value={searchInput}
        ...
        >
          ...
          <option value="unverified">
            {t("admin.filterStatusUnverified")}
          </option>
          ...
          <table className="admin-users-table">
            <thead>
              <tr>
                ...
                <th className="admin-users-actions-header">
                  {t("admin.userTableActions")}
                </th>
              </tr>
            </thead>
            <tbody>
              {users.map((user) => {
                ...
                return (
                  <tr
                    key={user._id}
                    className={status === "banned" ? "banned-row" : ""}
                  >
      ...
      {banModalOpen && (
        <div
          className="admin-ban-modal-overlay"
          onClick={() => setBanModalOpen(false)}
        >
          <div
            className="admin-ban-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="admin-ban-modal-header">
              <h3 className="admin-ban-modal-title">
                {t("admin.banUserTitle")}
              </h3>
              <button
                className="admin-ban-modal-close"
                onClick={() => setBanModalOpen(false)}
              >
                <svg
                  viewBox="0 0 24 24"
            ...
            <div className="admin-ban-modal-footer">
              <button
                className="admin-ban-modal-cancel"
                onClick={() => setBanModalOpen(false)}
              >
                {t("admin.cancelBtn")}
              </button>
      ...
      <ConfirmModal
        ...
        title={
          confirmAction === "unlock"
            ? t("admin.confirmUnlockUserTitle")
            : t("admin.confirmLockUserTitle")
        }
        message={
          confirmAction === "unlock"
            ? t("admin.confirmUnlockUser")
            : t("admin.confirmLockUser")
        }
        confirmText={
          confirmAction === "unlock"
            ? t("admin.unlockUser")
