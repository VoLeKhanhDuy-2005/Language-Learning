...
const BattleResult = ({
  ...
  const myStats = players.find((p) => p.userId === myId) || {
    score: 0,
    correctCount: 0,
  };
  const oppStats = players.find((p) => p.userId === opponentId) || {
    score: 0,
    correctCount: 0,
  };

  const myFinalScore =
    scores[myId] !== undefined ? scores[myId] : myStats.score;
  const oppFinalScore = opponentId
    ? scores[opponentId] !== undefined
      ? scores[opponentId]
      : oppStats.score
    : 0;
      ...
      <div className="battle-result-scores-box">
        ...
        <div className="battle-result-row">
          <div className="battle-result-player-info">
            <div
              className={`battle-result-player-avatar ${!user?.avatarUrl ? "avatar-placeholder" : ""}`}
            >
              {user?.avatarUrl ? (
                <img
                  src={user.avatarUrl}
            ...
            <div>
              <span className="battle-result-player-name">
                {user?.name || t("battle.you")}
              </span>
              <div
        ...
        {opponent && (
          <div className="battle-result-row">
            <div className="battle-result-player-info">
              <div
                className={`battle-result-player-avatar ${!opponent.avatarUrl ? "avatar-placeholder" : ""}`}
              >
                {opponent.avatarUrl ? (
                  <img
            ...
            <span className="battle-result-player-score">
              {oppFinalScore} pts
            </span>
          </div>
        )}
      ...
      <div className="battle-result-rewards">
        ...
        <div className="battle-reward-items">
          ...
          <div className="battle-reward-item">
            ...
            <span className="battle-reward-lbl">
              {t("battle.streakMaintained")}
            </span>
          </div>
