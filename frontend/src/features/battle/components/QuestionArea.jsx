    ...
    timerRef.current = setInterval(() => {
      ...
      if (remain <= 0) {
        ...
        const percent = Math.min(
          100,
          Math.max(0, (remain / totalDuration) * 100),
        );
    ...
    if (roundResult) {
      ...
      if (mode === "typing" && hasSubmitted) {
        const isCorrect =
          ...
          roundResult.correctAnswer.toLowerCase().trim();
        if (!isCorrect) {
          setShake(true);
        }
      }
  ...
  return (
    <div style={{ width: "100%" }}>
      <div className="battle-question-card">
        {/* Câu hỏi */}
        <span className="battle-question-term">{question.term}</span>

        {/* Gợi ý cho Typing mode */}
        {mode === "typing" && (
          <div className="battle-question-hint">
            {t("battle.hint")}: {question.firstChar}... ({question.answerLength}{" "}
            {t("battle.letters")})
            {question.answerPattern && question.answerPattern.length > 1 && (
              <span>
                {" "}
                - {t("battle.pattern")}: {question.answerPattern.join(", ")}
              </span>
            )}
          </div>
        )}
        ...
        {mode === "mcq" ? (
          ...
          <div className="battle-typing-container">
            <form
              ...
              className="battle-typing-input-wrapper"
            >
              <input
                type="text"
