...
export const useBattleSocket = () => {
  ...
  if (!context) {
    throw new Error(
      "useBattleSocket must be used within a BattleSocketProvider",
    );
  }
  return context;
};
...
export const BattleSocketProvider = ({ children }) => {
  ...
  const [opponentDisconnectTimeLeft, setOpponentDisconnectTimeLeft] =
    useState(15);
  ...
  useEffect(() => {
    ...
    socketInstance.on(
      "battle:starting",
      ({ countdownMs, mode: matchMode, total, matchId: serverMatchId }) => {
        resetGameState();
        setMode(matchMode);
    ...
    socketInstance.on(
      "battle:roundResult",
      ({ index, correctAnswer, scores: roundScores }) => {
    ...
    socketInstance.on(
      ...
      ({
        ...
        setMatchId(
          serverMatchId || sessionStorage.getItem("currentMatchId") || "",
        );
        setQuestion({
          term,
          options,
