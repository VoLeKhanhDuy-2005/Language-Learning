  ...
  useEffect(() => {
    const fetchMatchDetails = async () => {
      ...
      try {
        ...
        const matchData =
          response?.data && response?.success !== undefined
            ? response.data
            : response;
        ...
        if (matchData && matchData.players) {
          ...
          if (opponentPlayer && opponentPlayer.userId) {
            ...
            setOpponent({
              id: isObject
                ? opponentPlayer.userId._id || opponentPlayer.userId.id
                : opponentPlayer.userId,
              name: isObject
                ? opponentPlayer.userId.name
                : t("battle.anonymous"),
        ...
        <div className="battle-pregame-countdown">
          <div className="battle-pregame-vs">
            ...
            <div className="battle-vs-player">
              <div
                className={`battle-vs-avatar ${!user?.avatarUrl ? "avatar-placeholder" : ""}`}
              >
                {user?.avatarUrl ? (
                  <img
                    src={user.avatarUrl}
            ...
            <div className="battle-vs-player">
              <div
                className={`battle-vs-avatar ${!opponent?.avatarUrl ? "avatar-placeholder" : ""}`}
              >
                {opponent?.avatarUrl ? (
                  <img
              ...
              <span className="battle-vs-name">
                {opponent?.name || t("battle.waiting")}
              </span>
            </div>
          </div>

          <div className="battle-pregame-number">{preGameCountdown}</div>
  ...
  if (gameStatus === "abandoned") {
    return (
      <div className="battle-play-layout">
        <div className="battle-pregame-countdown">
          <h2
            className="battle-result-outcome lose"
            style={{ fontSize: "32px" }}
          >
            {t("battle.matchAbandoned")}
          </h2>
          <p className="battle-lobby-subtitle">
            {t("battle.redirectingToLobby")}
          </p>
        </div>
      </div>
    );
