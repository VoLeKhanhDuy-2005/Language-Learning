...
function DeckCard({ deck, cefrLevels = [], tags = [], onClick }) {
  ...
  const colorIndex = deck._id
    ? deck._id.charCodeAt(deck._id.length - 1) % fallbackColors.length
    : 0;
  ...
  return (
    <div className="deck-card" onClick={() => onClick && onClick(deck._id)}>
      ...
      <div
        ...
        style={{
          backgroundColor: deck.coverImage ? "transparent" : fallbackColor,
        }}
      >
        {deck.coverImage ? (
          <img
