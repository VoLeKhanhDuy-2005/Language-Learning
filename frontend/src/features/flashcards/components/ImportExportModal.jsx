...
function ImportExportModal({ deckId, topic, onClose, onImportSuccess }) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `vocab_${topic.name.replace(/\s+/g, "_")}_export.xlsx`,
      );
      ...
      console.error(err);
      setErrorMsg(
        err.response?.data?.message || t("userDeckDetail.errorExportFailed"),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleImportSubmit = async (e) => {
    ...
    try {
      ...
      if (!presignedRes.success || !presignedRes.data?.uploadUrl) {
        throw new Error(
          presignedRes.message || t("userDeckDetail.errorUploadFailed"),
        );
      }
      ...
      setErrorMsg(
        err.response?.data?.message ||
          err.message ||
          t("userDeckDetail.errorImportFailed"),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="import-export-modal-overlay" onClick={onClose}>
      <div
        className="import-export-modal-container"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="import-export-modal-header">
          <h3 className="import-export-modal-title">
            {t("userDeckDetail.importExportModalTitle")}
          </h3>
          <button
            className="import-export-close-btn"
            onClick={onClose}
            aria-label="Close"
          >
            <svg
        ...
        <div className="import-export-modal-body">
          {errorMsg && (
            <div className="import-export-error-box">{errorMsg}</div>
          )}
          {successMsg && (
            <div className="import-export-success-box">{successMsg}</div>
          )}
          ...
          {activeTab === "export" ? (
            ...
            <div className="export-tab-content">
              <p className="import-export-description">
                ...
                Xuất tất cả thẻ từ vựng trong chủ đề này ra file Excel (.xlsx)
                để sao lưu hoặc chỉnh sửa.
              </p>
            </div>
          ) : (
            // Nội dung Tab IMPORT
            <div className="import-tab-content">
              ...
              <form onSubmit={handleImportSubmit}>
                ...
                {!selectedFile ? (
                  ...
                  >
                    ...
                    <div className="file-upload-text">
                      {t("userDeckDetail.selectFileBtn")}
                    </div>
                ...
                <div className="import-form-group">
                  <label className="import-form-label">
                    {t("userDeckDetail.importModeLabel")}
                  </label>
                  <select
                  ...
                  >
                    <option value="append">
                      {t("userDeckDetail.importModeAppend")}
                    </option>
                    <option value="replace">
                      {t("userDeckDetail.importModeReplace")}
                    </option>
                    <option value="upsert">
                      {t("userDeckDetail.importModeUpsert")}
                    </option>
                  </select>
                </div>
              </form>
              ...
              {importResult && (
                <div className="import-summary-container">
                  <h4 className="import-summary-title">
                    {t("userDeckDetail.importResultsSummary")}
                  </h4>
                  <div className="import-summary-grid">
                    <div className="summary-item">
                      <span className="summary-label">
                        {t("userDeckDetail.importResultTotal", {
                          total: "",
                        }).replace(":", "")}
                      </span>
                    ...
                    <div className="summary-item">
                      <span className="summary-label">
                        {t("userDeckDetail.importResultInserted", {
                          inserted: "",
                        }).replace(":", "")}
                      </span>
                    ...
                    <div className="summary-item">
                      <span className="summary-label">
                        {t("userDeckDetail.importResultUpdated", {
                          updated: "",
                        }).replace(":", "")}
                      </span>
                      <span className="summary-value">
                    ...
                    <div className="summary-item">
                      <span className="summary-label">
                        {t("userDeckDetail.importResultSkipped", {
                          skipped: "",
                        }).replace(":", "")}
                      </span>
                      <span className="summary-value">
                    ...
                    <div className="summary-item">
                      <span className="summary-label">
                        {t("userDeckDetail.importResultFailed", {
                          failed: "",
                        }).replace(":", "")}
                      </span>
                  ...
                  {importResult.errors && importResult.errors.length > 0 && (
                    <div className="import-errors-container">
                      <h5 className="import-errors-title">
                        {t("userDeckDetail.importResultErrors")}
                      </h5>
