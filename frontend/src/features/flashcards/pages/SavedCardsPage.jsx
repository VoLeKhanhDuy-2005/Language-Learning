...
function SavedCardsPage({ onNavigate }) {
  ...
  const fetchSavedCards = async () => {
    ...
    setError("");
    try {
      const response = await getUserDueCardStates({
        hidden: false,
        starred: true,
        page,
        limit,
        ...
        <>
          <div className="saved-cards-grid">
            {savedItems.map((item) => {
              ...
              // Lấy audio US/UK
              const ukPhonetic = card.phonetics?.find(
                (p) =>
                  p.locale?.toLowerCase().includes("uk") ||
                  p.locale?.toLowerCase().includes("gb"),
              );
              const usPhonetic = card.phonetics?.find((p) =>
                p.locale?.toLowerCase().includes("us"),
              );
                ...
                <div key={item._id} className="saved-card-item">
                  ...
                  <div className="saved-card-phonetics-section">
                    ...
                    <div className="saved-card-ipa-row">
                      {ukPhonetic?.text && (
                        <span className="saved-card-ipa">
                          UK <span className="ipa-text">{ukPhonetic.text}</span>
                        </span>
                      )}
                      {usPhonetic?.text && (
                        <span className="saved-card-ipa">
                          US <span className="ipa-text">{usPhonetic.text}</span>
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Loại từ (pos) */}
                  {card.pos && (
                    <div className="saved-card-pos-badge">{card.pos}</div>
                  )}

                  {/* Nghĩa & Giải thích */}
                  <div className="saved-card-meaning-section">
                    <h3 className="saved-card-translation">
                      {card.translation}
                    </h3>
                    {explanationText && (
                      <p className="saved-card-explanation">
                        {explanationText}
                      </p>
                    )}
                  </div>
