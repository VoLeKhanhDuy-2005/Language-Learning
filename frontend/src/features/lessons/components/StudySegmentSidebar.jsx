...
const StudySegmentSidebar = ({
  ...
  const lessonProgressPct = Math.round(
    (completedSegmentsCount / segments.length) * 100,
  );
  ...
  const handleSelect = (idx) => {
    ...
    const maxSelectableIdx =
      firstIncompleteIdx === -1 ? segments.length - 1 : firstIncompleteIdx;
    ...
    <aside className="study-sidebar-aside">
      <div className="progress-list-card">
        ...
        <div className="right-progress-bar-container">
          <div
            className="right-progress-bar-fill"
        ...
        <div className="sidebar-segments-scroller">
          {segments.map((item, idx) => {
            ...
            const isLocked =
              firstIncompleteIdx !== -1 && idx > firstIncompleteIdx;
            ...
            return (
              ...
              >
                <div className="segment-card-header-row">
                  <span className="segment-order-number">
                  ...
                  {isFinished && score !== undefined && score !== null ? (
                    <span
                      className={`segment-score-badge ${score < 50 ? "score-red" : score < 80 ? "score-yellow" : "score-green"}`}
                    >
                      {Math.round(score)}
                    </span>
                  ) : isSelected ? (
                    <span
                      className="status-badge-icon studying"
                      title="Studying"
                    >
                      <svg
                        viewBox="0 0 24 24"
                ...
                <div className="segment-card-preview-body">
                  {isFinished ? (
                    <p className="segment-text-preview">
                      {item.segment.transcript.original}
                    </p>
                  ) : (
