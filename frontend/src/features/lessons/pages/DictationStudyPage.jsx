...
function DictationStudyPage({ lessonId, onNavigate }) {
  ...
  const cleanRefWords = refWords.map((w) =>
  ...
  const isSegmentCompleted = parsedWords.every(
    (w) => w.status === "correct" || w.status === "revealed",
  );
          ...
          {currentSegmentIndex < segments.length ? (
            <div className="study-dictation-wrapper">
              ...
              <div className="words-match-feedback-section">
                <div className="feedback-section-title">
                  {t("dictation.vocabularyHint")}
                </div>
                <div className="words-flex-grid">
                  {parsedWords.map((wordObj, idx) => {
                    let cardClass = "word-status-empty";
                    if (wordObj.status === "correct")
                      cardClass = "word-status-correct";
                    else if (wordObj.status === "incorrect")
                      cardClass = "word-status-incorrect";
                    else if (wordObj.status === "revealed")
                      cardClass = "word-status-revealed";
            ...
            <div className="dictation-completion-screen">
              ...
              <h3 className="completion-title">
                {t("dictation.completedTitle")}
              </h3>
              <p className="completion-subtitle">
                {t("dictation.completedSubtitle")}
              </p>
              <button
                onClick={() => onNavigate && onNavigate("/lessons")}
