...
function ShadowingStudyPage({ lessonId, onNavigate }) {
      ...
      try {
        ...
        if (segmentsRes.success && Array.isArray(segmentsRes.data)) {
          ...
          setCurrentSegmentIndex(
            firstIncompleteIdx !== -1 ? firstIncompleteIdx : 0,
          );
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (segments.length > 0 && segments[currentSegmentIndex]) {
      const currentProgress =
        segments[currentSegmentIndex].userProgress?.shadowing;
      setAttemptCount(currentProgress ? currentProgress.attemptCount || 1 : 1);
      setAccuracyScore(
        currentProgress
          ? Math.round(
              ((v) => v.reduce((sum, s) => sum + s, 0) / v.length)(
                Object.values(currentProgress.wordsAccuracy),
              ),
            )
          : null,
      );
  ...
  const handleToggleRecord = async () => {
    ...
    if (!isRecording) {
      ...
      try {
        ...
        const response = await patchSegmentProgress(
          lessonId,
          segment._id,
          payload,
        );
        ...
        if (response.success) {
          ...
          if (shadowingData) {
            setAccuracyScore(
              Math.round(
                ((v) => v.reduce((sum, s) => sum + s, 0) / v.length)(
                  Object.values(shadowingData.wordsAccuracy),
                ),
              ),
            );
  ...
  const strokeDashoffset =
    accuracyScore !== null ? 283 - (283 * accuracyScore) / 100 : 283;
          ...
          {currentSegmentIndex < segments.length ? (
            <div className="study-shadowing-wrapper">
              ...
              <div className="recording-controls-row">
                ...
                <div className="control-btn-wrapper">
                  ...
                  <span className="control-btn-label">
                    {isRecording
                      ? t("shadowing.recordStop")
                      : t("shadowing.recordStart")}
                  </span>
                </div>
                ...
                <div className="shadowing-status-alert-box">
                  {isRecording && (
                    <span className="recording-text">
                      {t("shadowing.recordingState")}
                    </span>
                  )}
                  {isProcessing && (
                    <span className="processing-text">
                      {t("shadowing.processing")}
                    </span>
                  )}
            ...
            <div className="shadowing-completion-screen">
              ...
              <h3 className="completion-title">
                {t("shadowing.completedTitle")}
              </h3>
              <p className="completion-subtitle">
                {t("shadowing.completedSubtitle")}
              </p>
              <button
                onClick={() => onNavigate && onNavigate("/lessons")}
                className="btn-back-lessons-list"
              >
                {t("shadowing.backToList")}
              </button>
            </div>
