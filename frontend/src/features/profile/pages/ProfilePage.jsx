...
function ProfilePage({ onNavigate }) {
  ...
  const loadProfileData = async () => {
    ...
    await Promise.all([
      ...
      safeGet(
        () => getBattleHistory(1, 10),
        setMiniBattleHistory,
        (r) => r.data?.items || [],
      ),
      (async () => {
        try {
          ...
          if (res?.success) {
            ...
            const learnedLessons = data.filter(
              (l) =>
                l.userProgress &&
                (l.userProgress.dictation?.status ||
                  l.userProgress.shadowing?.status),
            );
            const map = {};
            learnedLessons.forEach((l) => {
              map[l.lesson._id] = l.lesson;
            });
            setLessonsMap(map);
            ...
            const lessonsWithData = learnedLessons.map((l) => ({
              ...
              lastStudiedAt:
                l.userProgress.updatedAt || l.userProgress.createdAt,
            }));
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      if (!presignedRes.success || !presignedRes.data?.uploadUrl) {
        throw new Error(
          presignedRes.message || t("profile.avatarUploadFailed"),
        );
      }
          ...
          <div className="profile-stats-row">
            <div className="profile-stat-card">
              ...
              <span className="profile-stat-value">
                {stats.learnedLessons || 0}
              </span>
              <span className="profile-stat-label">
                {t("profile.statsLessons")}
              </span>
            </div>
            <div className="profile-stat-card">
              <div className="profile-stat-icon">
          ...
          {gamification && (
            <div className="profile-level-card">
              <div className="profile-level-header">
                ...
                <span className="profile-xp-pct">
                  {t("profile.xpPercent", {
                    pct: gamification.progressPct || 0,
                  })}
                </span>
              </div>
              ...
              <div className="profile-xp-details">
                <span className="profile-xp-detail">
                  {t("profile.xpProgress", {
                    current: gamification.xpIntoLevel || 0,
                    next: gamification.xpForNextLevel || 100,
                  })}
                </span>
                <span className="profile-xp-remaining">
                  {t("profile.xpRemaining", {
                    remaining:
                      (gamification.xpForNextLevel || 100) -
                      (gamification.xpIntoLevel || 0),
                  })}
                </span>
              </div>
            </div>
            ...
            <div className="profile-mini-list">
              {historyTab === "lessons" &&
                (lessonProgress.length > 0 ? (
                  <table className="profile-mini-table">
                    ...
                    <tbody>
                      {lessonProgress.slice(0, 5).map((lesson) => {
                        const dictationPct =
                          lesson.userProgress?.dictation?.progressPct || 0;
                        const shadowingPct =
                          lesson.userProgress?.shadowing?.progressPct || 0;
                        const dictationStatus =
                          lesson.userProgress?.dictation?.status;
                        const shadowingStatus =
                          lesson.userProgress?.shadowing?.status;
                        ...
                        return (
                          <tr key={lesson._id}>
                            <td className="profile-lesson-name">
                              {lesson.title}
                            </td>
                            <td>
                            ...
                            <td>
                              <span
                                className={`profile-status-badge ${status === "completed" ? "completed" : status === "in_progress" ? "in_progress" : "not_started"}`}
                              >
                                {status === "completed"
                                  ? t("profile.completed")
                                  : status === "in_progress"
                  ...
                  <div className="profile-mini-empty">
                    {t("profile.noLessonHistory")}
                  </div>
                ))}
                      ...
                      {miniBattleHistory.slice(0, 5).map((match, idx) => {
                        ...
                        return (
                          <tr key={match._id || idx}>
                            ...
                            <td>
                              {match.matchType === "queue"
                                ? t("battle.typeQueue")
                                : t("battle.typeInvite")}
                            </td>
                            <td>
                              <span
                                className={`profile-status-badge ${isDraw ? "draw" : isWinner ? "win" : "lose"}`}
                              >
                                {isDraw
                                  ? t("profile.draw")
                                  : isWinner
