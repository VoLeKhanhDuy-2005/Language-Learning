...
describe("Quản trị viên Admin - E2E Test bằng Selenium", () => {
  ...
  const loginAsAdmin = async () => {
    ...
    await driver.wait(
      until.elementLocated(By.className("admin-layout")),
      15000,
    );
  };
  ...
  it("đăng nhập admin thành công và kiểm thử validation / tạo mới bộ từ vựng (Deck)", async () => {
    try {
      ...
      const saveBtn = await driver.wait(
        until.elementLocated(By.className("admin-save-btn")),
        10000,
      );
      ...
      const errorSpan = await driver.wait(
        until.elementLocated(By.className("input-error-message")),
        10000,
      );
      let errorText = "";
      ...
      const cefrPills = await driver.findElements(
        By.className("admin-cefr-pill"),
      );
      ...
      const currentSaveBtn = await driver.findElement(
        By.className("admin-save-btn"),
      );
  ...
  it("kiểm thử validation và tạo mới bài học video (Lesson)", async () => {
    try {
      ...
      const titleInput = await driver.wait(
        until.elementLocated(By.id("lesson-title")),
        10000,
      );
      ...
      const errorAlert = await driver.wait(
        until.elementLocated(By.className("admin-alert error")),
        10000,
      );
      let errorText1 = "";
      await driver.wait(async () => {
      ...
      const currentSaveBtn1 = await driver.findElement(
        By.className("admin-save-btn"),
      );
      await driver.executeScript("arguments[0].click();", currentSaveBtn1);

      const errorAlert2 = await driver.wait(
        until.elementLocated(By.className("admin-alert error")),
        10000,
      );
      let errorText2 = "";
      ...
      // 6. Nhấn lưu bài học
      const currentSaveBtn2 = await driver.findElement(
        By.className("admin-save-btn"),
      );
