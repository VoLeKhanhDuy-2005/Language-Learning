...
/**
 * Lấy mã OTP xác thực email từ backend test API.
 */
const getOtpFromApi = async (email) => {
  const res = await fetch(
    `http://localhost:5000/api/v1/test/otp/${encodeURIComponent(email)}`,
  );
...
describe("Đối kháng từ vựng Battle - E2E Test bằng Selenium", () => {
  ...
  const registerAndLogin = async (driver, email, password) => {
    ...
    const submitBtn = await driver.findElement(
      By.className("signup-submit-btn"),
    );
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      otpCode = await getOtpFromApi(email);
      if (otpCode) break;
    ...
    const verifySubmitBtn = await driver.findElement(
      By.className("verify-submit-btn"),
    );
  ...
  it("giả lập hai người chơi kết nối, tạo phòng, tham gia đấu và hoàn thành trận đấu", async () => {
    try {
      ...
      const battleLinkA = await driverA.wait(
        until.elementLocated(By.css('a[href="/battle"]')),
        10000,
      );
      await battleLinkA.click();
      await driverA.wait(until.urlContains("/battle"), 10000);

      const battleLinkB = await driverB.wait(
        until.elementLocated(By.css('a[href="/battle"]')),
        10000,
      );
      ...
      const createRoomBtn = await driverA.wait(
        until.elementLocated(By.className("btn-battle-secondary")),
        10000,
      );
      await driverA.executeScript("arguments[0].click();", createRoomBtn);

      // 5. Đọc mã phòng từ RoomModal của Người chơi A
      const codeBox = await driverA.wait(
        until.elementLocated(By.className("battle-modal-code-box")),
        10000,
      );
      ...
      const inviteInput = await driverB.wait(
        until.elementLocated(By.className("battle-invite-code-input")),
        10000,
      );
      await inviteInput.sendKeys(roomCode);

      const joinBtn = await driverB.findElement(
        By.xpath(
          "//button[@type='submit' and contains(@class, 'btn-battle-primary')]",
        ),
      );
      ...
      for (let sec = 0; sec < maxWaitSec; sec++) {
        ...
        try {
          ...
          const resultsA = await driverA.findElements(
            By.className("battle-result-container"),
          );
          ...
          const optionsA = await driverA.findElements(
            By.className("battle-option-button"),
          );
          const optionsB = await driverB.findElements(
            By.className("battle-option-button"),
          );
          ...
          if (optionsA.length > 0) {
            ...
            if (!isDisabled) {
              console.log(
                `--- E2E Debug: Người chơi A click MCQ option ở giây thứ ${sec}`,
              );
          ...
          if (optionsB.length > 0) {
            ...
            if (!isDisabled) {
              console.log(
                `--- E2E Debug: Người chơi B click MCQ option ở giây thứ ${sec}`,
              );
          ...
          const typingInputsA = await driverA.findElements(
            By.className("battle-typing-input"),
          );
          const typingInputsB = await driverB.findElements(
            By.className("battle-typing-input"),
          );
          ...
          if (typingInputsA.length > 0) {
            ...
            if (!isDisabled) {
              console.log(
                `--- E2E Debug: Người chơi A gửi câu trả lời Typing ở giây thứ ${sec}`,
              );
              await typingInputsA[0].sendKeys("test");
              const submitBtnA = await driverA.findElement(
                By.css('.battle-typing-input-wrapper button[type="submit"]'),
              );
          ...
          if (typingInputsB.length > 0) {
            ...
            if (!isDisabled) {
              console.log(
                `--- E2E Debug: Người chơi B gửi câu trả lời Typing ở giây thứ ${sec}`,
              );
              await typingInputsB[0].sendKeys("test");
              const submitBtnB = await driverB.findElement(
                By.css('.battle-typing-input-wrapper button[type="submit"]'),
              );
          ...
          console.log(
            `--- E2E Debug: Gặp lỗi nhỏ trong vòng lặp ở giây thứ ${sec}: ${err.message}. Tiếp tục...`,
          );
        }
      }
      ...
      const exitBtnA = await driverA.wait(
        until.elementLocated(By.className("btn-battle-primary")),
        10000,
      );
      await driverA.executeScript("arguments[0].click();", exitBtnA);
      await driverA.wait(until.urlContains("/battle"), 10000);

      const exitBtnB = await driverB.wait(
        until.elementLocated(By.className("btn-battle-primary")),
        10000,
      );
