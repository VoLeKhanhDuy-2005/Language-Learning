  ...
  it("đăng nhập, truy cập danh sách bài học và hoàn thành bài học chính tả bằng gợi ý", async () => {
    try {
      ...
      for (let step = 0; step < maxSegments; step++) {
        ...
        try {
          await driver.wait(async () => {
            const spinners = await driver.findElements(
              By.className("dictation-spinner"),
            );
        ...
        const completionScreens = await driver.findElements(
          By.className("dictation-completion-screen"),
        );
        ...
        await driver.wait(async () => {
          const screens = await driver.findElements(
            By.className("dictation-completion-screen"),
          );
          ...
          try {
            const currentNextBtn = await driver.findElement(
              By.className("btn-primary-next"),
            );
