...
describe("Đăng ký & Xác thực OTP - E2E Test bằng Selenium", () => {
  ...
  it("đăng ký tài khoản mới và xác thực OTP qua Redis thành công", async () => {
    ...
    // 4. Lấy mã OTP từ backend test API
    let otpCode = null;
    for (let attempt = 0; attempt < 10; attempt++) {
      otpCode = await getOtpFromApi(uniqueEmail);
      if (otpCode) break;
    ...
    await driver.wait(
      until.elementLocated(By.className("verify-success-message")),
      10000,
    );
    ...
    const loginEmailInput = await driver.wait(
    ...
    const isLogged = await driver
      .findElement(By.className("header-user-btn"))
      .isDisplayed();
