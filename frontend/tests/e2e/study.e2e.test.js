...
describe("Học từ vựng & Đồng bộ Starred - E2E Test bằng Selenium", () => {
  ...
  it("đăng nhập, truy cập kho từ vựng và đồng bộ hóa trạng thái starred giữa Flashcard/Quiz", async () => {
    ...
    for (let i = 0; i < topics.length; i++) {
      if (i > 0) {
        const nextTopic = await driver.findElements(
          By.className("topic-item-card"),
        );
        ...
        try {
          await driver.wait(
            until.elementLocated(By.className("study-loading-zone")),
            800,
          );
        } catch (e) {
        ...
        await driver.wait(async () => {
          const loadingZones = await driver.findElements(
            By.className("study-loading-zone"),
          );
      ...
      const cardsZone = await driver.findElements(
        By.className("flashcard-card"),
      );
    ...
    if (!flashcardFound) {
      console.log(
        "--- E2E Info: Toàn bộ từ vựng trong bộ từ đã được học hết. Kết thúc sớm test case. ---",
      );
      return;
    }
    ...
    const starBtn = await driver.findElement(
      By.className("flashcard-star-btn"),
    );
    ...
    try {
      ...
      await driver.wait(async () => {
        const activeStarBtn = await driver.findElement(
          By.className("flashcard-star-btn"),
        );
        starredClassAfterClick = await activeStarBtn.getAttribute("class");
        return (
          starredClassAfterClick.includes("starred") === !isInitiallyStarred
        );
      }, 10000);
      console.log(
        "--- E2E Debug: starredClassAfterClick =",
        starredClassAfterClick,
      );
      expect(starredClassAfterClick.includes("starred")).toBe(
        !isInitiallyStarred,
      );
    } catch (err) {
    ...
    const quizModeBtn = await driver.findElement(
    ...
    await driver.wait(
      until.elementLocated(By.className("quiz-card-container")),
      10000,
    );
    ...
    await driver.wait(async () => {
      const activeQuizStarBtn = await driver.findElement(
        By.className("quiz-star-btn"),
      );
      quizStarredClassAfterClick =
        await activeQuizStarBtn.getAttribute("class");
      return (
        quizStarredClassAfterClick.includes("starred") === isInitiallyStarred
      );
    }, 10000);
    expect(quizStarredClassAfterClick.includes("starred")).toBe(
      isInitiallyStarred,
    );

    // 11. Chuyển ngược lại chế độ Flashcard
    const flashcardModeBtn = await driver.findElement(
      By.xpath("//button[contains(text(), 'FlashCard')]"),
    );
    await flashcardModeBtn.click();
    await driver.wait(
      until.elementLocated(By.className("flashcard-card")),
      10000,
    );

    // 12. Xác minh ngôi sao ở Flashcard đã được đồng bộ ngược lại chính xác
    const starBtnFinal = await driver.findElement(
      By.className("flashcard-star-btn"),
    );
    ...
    const flashcardToFlip = await driver.findElement(
      By.className("flashcard-card"),
    );
