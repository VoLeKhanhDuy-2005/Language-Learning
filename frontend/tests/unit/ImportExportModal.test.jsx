...
describe("ImportExportModal Component", () => {
  ...
  it("render modal chính xác ở tab Xuất mặc định", () => {
    ...
    expect(
      screen.getByText("userDeckDetail.importExportModalTitle"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/userDeckDetail.cardsInTopic topicName:IELTS Wordlist/),
    ).toBeInTheDocument();
  ...
  it("chuyển tab hoạt động bình thường", async () => {
    ...
    expect(
      screen.getByText("userDeckDetail.downloadTemplateBtn"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("userDeckDetail.importModeLabel"),
    ).toBeInTheDocument();
  ...
  it("gọi API export khi click nút xuất Excel", async () => {
    const mockExportBlob = new Blob(["dummy excel data"], {
    ...
    expect(api.exportUserTopicCards).toHaveBeenCalledWith(
      mockDeckId,
      mockTopic._id,
    );
  ...
  it("gọi API import và hiển thị kết quả sau khi nhập thành công", async () => {
    ...
    expect(api.importUserTopicCards).toHaveBeenCalledWith(
      mockDeckId,
      mockTopic._id,
      {
    ...
    expect(
      await screen.findByText("userDeckDetail.importSuccessMsg"),
    ).toBeInTheDocument();
